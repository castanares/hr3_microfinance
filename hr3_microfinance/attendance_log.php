...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadAttendance(){
        try {
            const [attendanceRes, shiftsRes, employeesRes] = await Promise.all([
                fetch(API_URL),
                fetch(SHIFTS_API),
                fetch(EMPLOYEES_API)
            ]);
            ...
            if (Array.isArray(employeesData)) {
                employeesData.forEach(emp => {
                    const id = emp.id || emp.employee_id || emp.ID || emp.emp_id;
                    if (id !== undefined && id !== null) employeeNameMap[String(id)] = emp.name || emp.full_name || emp.employee_name || (emp.firstname ? emp.firstname + ' ' + (emp.lastname||'') : '');
                });
            }

            const filterDate = filterDateEl.value || null;
            const activeDate = filterDate || new Date().toISOString().split('T')[0];

            tableBody.innerHTML = "";
            let cnt = 0;
            // Build a map of attendance by employee+date+shift for quick checking
            const attendanceMap = {};
            attendanceData.forEach((row) => {
                ...
                // store into map
                const key = `${row.employee_id}::${dateVal}::${shiftName}`;
                ...
                tr.innerHTML = `
                    ...
                    <td class="px-4 py-2 border border-gray-200 font-semibold ${row.status_clock_out || row.status === 'Present' ? 'text-green-600' : row.status_clock_out || row.status === 'Late' ? 'text-yellow-600' : 'text-red-600'}">${row.status_clock_out || row.status || ''}</td>
                `;
            ...
            // Now append Absent rows for any scheduled shifts on the active date where the
            // employee has no valid clock-in/out AND the shift end time has already passed.
            try {
                const now = new Date();
                shiftsData.forEach(shift => {
                    try {
                        if (!shift || !shift.date) return;
                        if (shift.date !== activeDate) return; // only consider the active date
                        const empId = shift.employee_id || shift.emp_id || shift.employeeId || shift.id;
                        const shiftName = shift.shift_name || shift.shift || '-';
                        const key = `${empId}::${shift.date}::${shiftName}`;
                        const existing = attendanceMap[key];
                        // If record exists and has both time_in and time_out, skip
                        if (existing && existing.time_in && existing.time_out) return;

                        // If already an attendance without full times, still consider absent if end time passed
                        const endDate = parseShiftTimeToDate(shift.end_time || shift.time_end || shift.end || shift.endtime, now);
                        if (!endDate) return;
                        if (now <= endDate) return; // shift not yet ended

                        // Render an Absent row for this shift
                        cnt++;
                        const empName = (shift.employee_name || employeeNameMap[String(empId)] || 'Unknown');
                        const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
                        ...
                        tr.innerHTML = `
                            <td class="px-4 py-2 border border-gray-200">${cnt}</td>
                            <td class="px-4 py-2 border border-gray-200">${empName}</td>
                            <td class="px-4 py-2 border border-gray-200">${shiftName}</td>
                            <td class="px-4 py-2 border border-gray-200">${formattedDate}</td>
                            <td class="px-4 py-2 border border-gray-200">-</td>
                            <td class="px-4 py-2 border border-gray-200 font-semibold text-red-600">Absent</td>
                ...
                console.warn('Error appending absent rows', e);
            }
        } catch (err) {
        ...
        try {
            ...
            if (!existingAtt || !existingAtt.time_in) {
                ...
                if (endDate && now < endDate) {
                    // Block locally as well to avoid unnecessary API calls
                    showAlert('Shift not ended yet.', 'warning');
                    prepareNextScan();
                    return;
                }
