...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function submitRfid(e) {
        ...
        try {
            ...
            // Hardened prevention: if there's any attendance today for this employee with time_in and no time_out,
            // block another scan until shift end. This avoids double attendance even if shift names differ.
            if (Array.isArray(attData)) {
                const anyOpen = attData.some(a => a.time_in && !a.time_out && String(a.date || '') === currentDate);
                if (anyOpen) {
                    // Determine shift end time (prefer today's shift record we fetched earlier)
                    ...
                    if (endDateObj && now < endDateObj) {
                        showAlert('Employee already clocked in today and it is not yet time to clock out.', 'error', 5000);
                        prepareNextScan();
                        return;
                    }
                    // If we couldn't parse shift end time, block by default to avoid duplicates
                    showAlert('Employee already clocked in today. Please contact admin to force clock-out.', 'error', 5000);
                    prepareNextScan();
                    return;
                }
                ...
                const shiftEndRaw = shift.end_time || shift.time_end || shift.end || null;
                let endDateObj = null;
                if (shiftEndRaw) endDateObj = parseShiftTimeToDate(shiftEndRaw, new Date());
                const now = new Date();
                if (endDateObj && now < endDateObj) {
                    showAlert('Employee already clocked in today and it is not yet time to clock out.', 'error', 5000);
                    prepareNextScan();
